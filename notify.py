#!/usr/bin/env python3
"""arc-copybot Telegram notifier — a SEPARATE process. The bot never talks to
...
BOT_UNIT = os.environ.get("BOT_UNIT", "arc-copybot")  # systemd unit whose journal is relayed
...
def view_logs(n=25):
    try:
        return subprocess.run(["journalctl", "-u", BOT_UNIT, "-n", str(n), "-o", "cat", "--no-pager"],
            ...
            for u in updates(st["offset"]):
                ...
                if st.get("chat_id") is None:
                    if text.startswith("/start") and code in text:
                        st["chat_id"] = cid
                        save_state(st)
                        send(cid, f"paired. this chat now receives the {BOT_UNIT} stream.\n\n" + HELP)
...
def stream_loop(st):
    ...
    proc = subprocess.Popen(["journalctl", "-fu", BOT_UNIT, "-o", "cat", "-n", "0"],
