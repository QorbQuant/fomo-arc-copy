#!/usr/bin/env python3
"""arc-copybot dashboard: live + paper performance in one auto-refreshing terminal screen.
...
INSTANCES = [
    ("LIVE", _HERE),
    ("PAPER", _HERE.parent / "arc-copybot-paper"),  # optional second instance; skipped if absent
]
USDC = "0x3600000000000000000000000000000000000000"
RPC = "https://rpc.mainnet.arc.io"
REFRESH = 15
_s = requests.Session()
_s.headers["User-Agent"] = "arc-copybot-dash/0.1"
...
def prices(tokens):
    ...
    for i in range(0, len(toks), 30):
        try:
            ...
            pairs = [p for p in (r.json().get("pairs") or []) if p.get("chainId") == "arc"]
        except Exception:
            continue
        best = {}
        for p in pairs:
        ...
        if t not in out:
            try:
                ...
                cands = [(((p.get("liquidity") or {}).get("usd") or 0), float(p["priceUsd"]))
                         ...
                         if p.get("chainId") == "arc" and p.get("priceUsd")
...
def snapshot():
    ...
    for name, d, cfg, st, sigs, n_all, n_bought in loaded:
        ...
        for pos in sorted(st["positions"].values(), key=lambda p: -p["bought_at"]):
            ...
            rows.append({"sym": pos["symbol"], "in": pos["buy_usd"], "held": value, "sold": pos["usdc_out"],
...
def dex_url(token):
    return f"https://dexscreener.com/arc/{token}"
...
def draw(stdscr, snap, sel=None, notice=""):
    ...
    title = (f" arc-copybot dashboard  {time.strftime('%H:%M:%S', time.localtime(snap['ts']))}  "
...
def main_curses(stdscr):
    ...
    while True:
        if ref.snap is None:
            stdscr.erase()
            stdscr.addnstr(0, 0, " arc-copybot dashboard   loading...", stdscr.getmaxyx()[1] - 1, curses.A_REVERSE)
            stdscr.refresh()
...
def main_once():
    snap = snapshot()
    print(f"arc-copybot dashboard  {time.strftime('%H:%M:%S', time.localtime(snap['ts']))}")
    for inst in snap["inst"]:
        print()
